...
const Carousel = ({ recentAnime }) => {
  ...
  return (
    <div className="carousel-wrapper group">
      <div className="hidden group-hover:block absolute top-[50%] -translate-y-[-50%] left-8 cursor-pointer z-10">
        <FaChevronLeft
          size={30}
          onClick={() => {
      ...
      <div className="hidden group-hover:block absolute top-[50%] -translate-y-[-50%] right-8 cursor-pointer z-10">
        <FaChevronRight
          size={30}
          onClick={() => {
            updateIndex(activeIndex + 1);
      ...
      >
        {recentAnime.map((anime, i) => (
          <div id="slide" key={i} className="relative flex-none w-full h-full">
            ...
            {/* slide content */}
            <div className="absolute z-10 top-1/3 left-20 right-20 leading-8 select-none text-white">
              <div className="text-3xl font-bold mb-2">{anime.title}</div>
              <div className="flex gap-2">
                <p className="font-semibold">{anime.year}</p>
                <p className="font-bold bg-zinc-700 rounded-sm px-2">
                  {anime.rating}
                </p>
              </div>
              <p>
